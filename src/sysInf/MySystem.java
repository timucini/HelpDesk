 ...
 * @version 2017-06-15
 * Stellt ein Objekt mit vielen - mit Java möglichen - Systeminformationen 
 * zur Verfügung.
 *
 */
public class MySystem {
	private static Map<String, String> env;
	protected double dFSrootCap, dFSrootFree, dFSrootUsed;
	protected String sPCName, sLogonServer, sUserName, sOS, 
					sProcessor, sHomeDrive;
	protected int iNrOfCPUs;
	...
	private void loadSysInf(){
		...
		sPCName=getComputername();
		sLogonServer=getLogonServer(); 
		sUserName=getUserName(); 
		sOS=getOS();
		sProcessor=getProcessor();
		sHomeDrive=getSysDrive();
		iNrOfCPUs=getNumberOfCPU();
		dFSrootCap=getSysDriveCap();
		dFSrootFree=getSysDriveFree();
		dFSrootUsed=getSysDriveUsed();
	}
		...
		public Double getSysDriveCap(){
			return (Double) dFSrootCap/Math.pow(1024, 2);
		}
		...
		public Double getSysDriveFree(){
			return (Double) dFSrootFree/Math.pow(1024, 2);
		}
		...
		public Double getSysDriveUsed(){
			return (Double) (dFSrootCap-dFSrootFree)/Math.pow(1024, 2);
		}
